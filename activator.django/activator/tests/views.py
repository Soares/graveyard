...
def url(name, *args):
    type = SimpleRequest().type
    args = (type,) + args
    return reverse('activator.views.%s' % name, args=args)
...
class ViewTests(TestCase):
    ...
    def test_request(self):
        ...
        self.assertEqual(response.template.name, 'activator/simple.html')

        response = self.client.post(url('request'), {'email': 'user@test.com'})
        self.assertEqual(response.status_code, 302)
        self.assertEqual(SimpleRequest.objects.count(), 1)
        self.assertEqual(SimpleRequest.objects.get().user, self.u)
        self.failUnless(response['location'].endswith(url('requested')))

    def test_requested(self):
        response = self.client.get(url('requested'))
        self.assertEqual(response.template.name, 'activator/simple-sent.html')

    def test_activate(self):
        record = SimpleRequest.objects.request(self.u)
        response = self.client.get(url('activate', record.token))
        self.assertEqual(response.status_code, 302)
        self.failUnless(SimpleRequest.objects.get().used)

        self.failUnless(response['location'].endswith(url('activated')))
        response = self.client.get(url('activated'))
        self.assertEqual(response.template.name, 'activator/simple-confirmed.html')
