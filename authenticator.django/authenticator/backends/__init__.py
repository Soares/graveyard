...
from authenticator.models import ExternalAuth
from authenticator.store import DjangoOpenIDStore
from authenticator.signals import pre_update, socialauth_registered
...
# Change the user details given a user and a details dict
def update_user_details(user, details):
    changed = False
    if not getattr(user, 'name', None):
        user.name = details['nickname'] or _fullname(details)
        changed = True
    if details['email'] not in user.emails:
        user.add_email(details['email'])
    return changed

# get the full name from a details dict
def _fullname(details):
    return details['fullname'] or (
...
CREATE_USERS = _setting('AUTHENTICATOR_CREATE_USERS', True)
LOAD_EXTRA_DATA = _setting('AUTHENTICATOR_EXTRA_DATA', True)
CHANGE_SIGNAL_ONLY = _setting('AUTHENTICATOR_CHANGE_SIGNAL_ONLY', False)
UPDATE_USER_DETAILS = _setting('AUTHENTICATOR_UPDATE_USER_DETAILS',
        update_user_details)
...
class SocialAuthBackend(ModelBackend):
    ...
    def authenticate(self, *args, **kwargs):
        ...
        # don't match the email/password calling conventions of
        # authenticate.
        ...
        is_new = False
        try:
            social_user = ExternalAuth.objects.select_related('user').get(
                    provider=self.name, uid=uid)
        except ExternalAuth.DoesNotExist:
            ...
            # much intrusive
            if 'user' in kwargs and kwargs['user'] != social_user.group.user:
                raise ValueError('Account already in use.', social_user)
            user = social_user.group.user
    ...
    def associate_auth(self, user, uid, response, details):
        ...
        email = details.get('email') or None
        group, created = user.authgroups.get_or_create(email=email)
        return ExternalAuth.objects.create(
            group=group,
            uid=uid,
            provider=self.name,
            fullname=_fullname(details),
    ...
    def update_user_details(self, user, response, details, is_new=False):
        """Update user details with (maybe) new data. Email is not
        changed if associating a new credential."""
        changed = False  # flag to track changes

        # check if values update should be left to signals handlers only
        if UPDATE_USER_DETAILS and not CHANGE_SIGNAL_ONLY:
            changed = UPDATE_USER_DETAILS(user, details) != False
...
class OpenIDBackend(SocialAuthBackend):
    ...
    def get_user_details(self, response):
        ...
        values = {'email': '', 'nickname': '', 'fullname': '',
...
class BaseAuth(object):
    ...
    def disconnect(self, user):
        ...
        """
        user.authenticators.filter(provider=self.AUTH_BACKEND.name).delete()
...
AUTHENTICATOR_IMPORT_SOURCES = (
    'authenticator.backends',
    'authenticator.backends.contrib',
) + _setting('AUTHENTICATOR_IMPORT_BACKENDS', ())

def get_backends():
    backends = {}

    for mod_name in AUTHENTICATOR_IMPORT_SOURCES:
        try:
